...
func void DIA_Lester_BACKINTOWN_Info ()
{
	...
	AI_StopProcessInfos (self);
	Npc_ExchangeRoutine (self,"XardasLibrary");	//Joly: Lester geht wieder zurück in sein Tal!
};
...
INSTANCE DIA_PC_Psionic_PICKPOCKET (C_INFO)
{
	...
	permanent	= TRUE;
	description = "(It will be easy to steal this key.)";
};                       
...
FUNC INT DIA_PC_Psionic_PICKPOCKET_Condition()
{
	...
	&& (Npc_HasItems(self, ITKE_REVIVED_STONEFORTRESS) >= 1)
	&& (LesterKeyStolen != TRUE)
	&& (other.attribute[ATR_DEXTERITY] >= 0)
	{
		return TRUE;
	};
};
...
{
	if (other.attribute[ATR_DEXTERITY] >= 0)
	{
		B_GiveInvItems (self, other, ITKE_REVIVED_STONEFORTRESS, 1);
